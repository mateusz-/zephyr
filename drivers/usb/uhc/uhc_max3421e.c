...
static K_KERNEL_STACK_DEFINE(drv_stack, 512);
					  ...
					  const uint8_t addr)
{
	...
	if (priv->addr != addr) {
		priv->addr = addr;
		ret = max3421e_write_byte(dev, MAX3421E_REG_PERADDR, priv->addr);
	}
