...
/**
 * @brief These macros SET, CLEAR, and CHECK, a Type-C Layer Flag
 */
 ...
 */
enum tc_flags {
	/** Flag to track Rp resistor change when the sink attached sub-state runs */
...
static void tc_set_state(const struct device *dev, enum tc_state_t state);
static enum tc_state_t tc_get_state(const struct device *dev);
static void pd_enable(const struct device *dev, bool enable);
...
void tc_run(const struct device *dev, int32_t dpm_request)
{
	...
	switch (port->tc_sm_state) {
	case SM_PAUSED:
		if (port->tc_enabled == false) {
			break;
		}
	/* fall through */
	case SM_INIT:
		...
		tc_init(dev);
		port->tc_sm_state = SM_RUN;
	/* fall through */
	case SM_RUN:
		if (port->tc_enabled == false) {
			pd_enable(dev, false);
			port->tc_sm_state = SM_PAUSED;
			break;
		}
...
static enum tc_state_t tc_get_state(const struct device *dev)
{
	...
	return port->tc->ctx.current - &tc_snk_states[0];
}
...
static void sink_power_sub_states(const struct device *dev)
{
	...
	enum policy_check_t dpm_pwr_change_notify;
	struct tc_sm_t *tc = port->tc;
	...
	if (cc == TC_CC_VOLT_RP_DEF) {
		...
		/*
		 * This sub-state supports Sinks consuming current within the
		 * two lower ranges (default and 1.5 A) of Source-supplied current.
		 */
		new_cc_voltage = TC_CC_VOLT_RP_1A5;
		dpm_pwr_change_notify = POWER_CHANGE_1A5;
	} else if (cc == TC_CC_VOLT_RP_3A0) {
		/*
		 * This sub-state supports Sinks consuming current within all three
		 * ranges (default, 1.5 A and 3.0 A) of Source-supplied current.
		 */
	...
	if (new_cc_voltage != tc->cc_voltage) {
		...
		SET_FLAG(tc, TC_FLAGS_RP_SUBSTATE_CHANGE);
	...
	if (CHK_AND_CLR_FLAG(tc, TC_FLAGS_RP_SUBSTATE_CHANGE)) {
		if (port->policy_cb_notify) {
			port->policy_cb_notify(dev, dpm_pwr_change_notify);
		}
	}
}
...
static void tc_attach_wait_snk_run(void *obj)
{
	 ...
	 * VBUS is detected.
	 */
	vbus_present = vbus_check_level(vbus, TC_VBUS_PRESENT);
...
static void tc_attached_snk_run(void *obj)
{
	...
	/* Detach detection */
	if (vbus_check_level(vbus, TC_VBUS_PRESENT) == false) {
		tc_set_state(dev, TC_UNATTACHED_SNK_STATE);
		return;
	}
