...
struct policy_engine {
	...
	/** DPM request */
	enum policy_request_t dpm_request;
...
/**
 * @brief Run the Policy Engine Layer state machine. This is called from the subsystems
 *	  port stack thread
 *
 * @param dev Pointer to the device structure for the driver instance
...
/**
 * @brief Informs the Policy Engine that a transmit message was discarded because
...
/*
 * @brief Informs the Policy Engine that it should invalidate the explicit contract.
 *
 * @param dev Pointer to the device structure for the driver instance
 */
...
/**
 * @brief Return true if the PE is is within an atomic messaging sequence that it
 *	  initiated with a SOP* port partner.
 *
