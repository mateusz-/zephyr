...
static int lb_control(struct usbd_class_node *c_nd,
		      ...
		      struct net_buf *const buf)
{
	...
	if (usb_reqtype_is_to_device(setup) &&
	    setup->bRequest == LB_VENDOR_REQ_OUT) {
	...
	if (usb_reqtype_is_to_host(setup) &&
	    setup->bRequest == LB_VENDOR_REQ_IN) {
		struct net_buf *buf;

		buf = usbd_ep_ctrl_buf_alloc(uds_ctx, USB_CONTROL_EP_IN,
					     setup->wLength);
		if (buf == NULL) {
			return -ENOMEM;
		}
