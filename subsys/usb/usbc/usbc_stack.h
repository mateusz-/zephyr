...
struct usbc_port_config {
	/** The usbc stack initializes this pointer that creates the main thread for this port */
...
struct usbc_port_data {
	...
	/**
	 * Callback used by the Policy Engine to ask the Device Policy Manager if a particular
	 * policy should be allowed
	 */
	bool (*policy_cb_check)(const struct device *dev, enum policy_check_t policy_check);
	/**
	 * Callback used by the Policy Engine to notify the Device Policy Manager of a policy
	 * change
	 */
	void (*policy_cb_notify)(const struct device *dev, enum policy_notify_t policy_notify);
	/**
	 * Callback used by the Policy Engine to notify the Device Policy Manager of WAIT
	 * message reception
	 */
	bool (*policy_cb_wait_notify)(const struct device *dev, enum policy_wait_t policy_notify);
	/**
	 * Callback used by the Policy Engine to get the Sink Capabilities from the
	 * Device Policy Manager
	 */
	int (*policy_cb_get_snk_cap)(const struct device *dev, uint32_t **pdos, int *num_pdos);
	/**
	 * Callback used by the Policy Engine to send the received Source Capabilities to the
	 * Device Policy Manager
	 */
	void (*policy_cb_set_src_cap)(const struct device *dev, uint32_t *pdos, int num_pdos);
	/**
	 * Callback used by the Policy Engine to get the Request Data Object (RDO) from the
	 * Device Policy Manager
	 */
	uint32_t (*policy_cb_get_request_data_object)(const struct device *dev);
	/** Callback used by the Policy Engine to check if Sink Power Supply is at default level */
	bool (*policy_cb_is_snk_at_default_level)(const struct device *dev);
