...
using the USB-C subsystem. The application implements a USBC Sink device.
...
.. _usbc-sink-sample-requirements:

Requirements
************
The TCPC device used by the sample is specified in the devicetree
node that's compatible with ``usbc-port``.
...
.. zephyr-app-commands::
   :zephyr-app: samples/subsys/usbc/sink
   :board: b_g474e_dpow1
   :goals: build flash
   :compact:
