 ...
 */
static inline int usbd_class_control(struct usbd_class_node *const node,
				     ...
				     struct net_buf *const buf)
{
	...
	if (api->control != NULL) {
		return api->control(node, setup, buf);
	}
...
/**
 * @brief Start of Frame (SoF) handler
 *
 * If available this handler is called for each SoF.
 * Can be used to prepare isochronous transfers.
 *
