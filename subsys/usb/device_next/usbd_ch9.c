...
/* TODO: Add to a public interface for completion callback. */
static void ch9_set_completion_cb(struct usbd_contex *const uds_ctx,
				  void *const cb, void *const data)
{
	uds_ctx->ch9_data.completion_cb = cb;
	uds_ctx->ch9_data.completion_data = data;
}
...
static int sreq_set_address(struct usbd_contex *const uds_ctx)
{
	...
	ch9_set_completion_cb(uds_ctx, sreq_set_address_cb, NULL);
			  ...
			  struct net_buf *const dbuf)
{
	...
	case USB_REQTYPE_RECIPIENT_ENDPOINT:
		...
		if (c_nd != NULL) {
			ret = usbd_class_control(c_nd, setup, dbuf);
		} else {
			errno = -ENOTSUP;
		}
		break;
			  ...
			  struct net_buf *const buf, const int err)
{
	...
	if (bi->setup && bi->ep == USB_CONTROL_EP_OUT) {
		...
		ch9_set_completion_cb(uds_ctx, NULL, NULL);
	...
	if (bi->status && bi->ep == USB_CONTROL_EP_IN) {
		if (ch9_get_ctrl_type(uds_ctx) == CTRL_AWAIT_STATUS_STAGE) {
			...
			if (uds_ctx->ch9_data.completion_cb) {
				return uds_ctx->ch9_data.completion_cb(uds_ctx, NULL);
			}
		} else {
