...
{
	struct udc_data *data = dev->data;
	const uint8_t idx = USB_EP_DIR_IS_IN(ep) ?
			    (ep & BIT_MASK(4)) + 16 :
			    ep & BIT_MASK(4);

	return data->ep_lut[idx];
}
...
int udc_register_ep(const struct device *dev, struct udc_ep_config *const cfg)
{
	...
	idx = USB_EP_DIR_IS_IN(cfg->addr) ? (cfg->addr & BIT_MASK(4)) + 16 :
					    cfg->addr & BIT_MASK(4);
	if (idx > ARRAY_SIZE(data->ep_lut)) {
		return -EINVAL;
	}
...
static bool udc_data_stage_to_device(const struct net_buf *const buf)
{
	return !udc_data_stage_to_host(buf);
}
			   ...
			   struct net_buf *const buf)
{
	...
	if (bi->setup && bi->ep == USB_CONTROL_EP_OUT) {
		 ...
		 */
		if (length && udc_data_stage_to_device(buf)) {
			/*
		...
		} else if (udc_data_stage_to_device(buf)) {
			/*
