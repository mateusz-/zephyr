...
/**
 * @brief These macros SET, CLEAR, and CHECK, a Protocol Layer Flag
 */
...
enum prl_flags {
	 ...
	 * when the message is sent to the TCPC layer.
	 */
...
static void prl_hr_set_state(const struct device *dev, enum usbc_prl_hr_state_t state);
static void prl_tx_set_state(const struct device *dev, enum usbc_prl_tx_state_t state);
static void prl_init(const struct device *dev);
static enum usbc_prl_hr_state_t prl_hr_get_state(const struct device *dev);

/**
 * @brief Initializes the TX an HR state machines and enters the PRL_TX_SUSPEND and
 *	  PRL_TX_SUSPEND states respectively.
 */
void prl_subsys_init(const struct device *dev)
{
	...
	struct protocol_hard_reset_t *prl_hr = port->prl_hr;
...
bool prl_is_running(const struct device *dev)
{
	...
	return port->prl_sm_state == SM_RUN;
}
...
void prl_execute_hard_reset(const struct device *dev)
{
	...
	SET_FLAG(prl_hr, PRL_FLAGS_PE_HARD_RESET);
	prl_hr_set_state(dev, PRL_HR_RESET_LAYER);
}

/**
 * @brief Instructs the Protocol Layer that a hard reset is complete. This function
 *	  is called from the Policy Engine.
 */
void prl_hard_reset_complete(const struct device *dev)
{
	...
	SET_FLAG(prl_hr, PRL_FLAGS_HARD_RESET_COMPLETE);
}
			   ...
			   enum pd_ctrl_msg_type msg)
{
	...
	SET_FLAG(prl_tx, PRL_FLAGS_MSG_XMIT);
}
...
void prl_set_default_pd_revision(const struct device *dev)
{
	 ...
	 */
	port->rev[PD_PACKET_SOP] = PD_REV30;
	port->rev[PD_PACKET_SOP_PRIME] = PD_REV30;
	port->rev[PD_PACKET_PRIME_PRIME] = PD_REV30;
	port->rev[PD_PACKET_DEBUG_PRIME] = PD_REV30;
	port->rev[PD_PACKET_DEBUG_PRIME_PRIME] = PD_REV30;
}
...
void prl_run(const struct device *dev)
{
	...
	switch (port->prl_sm_state) {
	case SM_PAUSED:
		if (port->prl_enabled == false) {
			break;
		}
	/* fall through */
	case SM_INIT:
		prl_init(dev);
		port->prl_sm_state = SM_RUN;
	/* fall through */
	case SM_RUN:
		if (port->prl_enabled == false) {
			port->prl_sm_state = SM_PAUSED;
			/* Disable RX */
			tcpc_set_rx_enable(port->tcpc, false);
			break;
		}
		...
		/*
		 * During Hard Reset no USB Power Delivery Protocol Messages are
		 * sent or received; only Hard Reset Signaling is present after
		 * which the communication channel is assumed to have been disabled
		 * by the Physical Layer until completion of the Hard Reset.
		 */
...
/**
 * @brief Set revision for the give packet type. This function is called from the Policy Engine.
 */
void prl_set_rev(const struct device *dev, enum pd_packet_type type, enum pd_rev_type rev)
{
	struct usbc_port_data *port = dev->data;
...
/**
 * @brief Get the revision for the give packet type. This function is called from the Policy Engine.
 */
enum pd_rev_type prl_get_rev(const struct device *dev, enum pd_packet_type type)
{
	struct usbc_port_data *port = dev->data;

	return port->rev[type];
}
...
static void alert_handler(const struct device *tcpc, void *port_dev, enum tcpc_alert alert)
{
	...
	case TCPC_ALERT_HARD_RESET_RECEIVED:
		SET_FLAG(prl_hr, PRL_FLAGS_PORT_PARTNER_HARD_RESET);
		break;
	case TCPC_ALERT_TRANSMIT_MSG_FAILED:
		SET_FLAG(prl_tx, PRL_FLAGS_TX_ERROR);
		break;
	case TCPC_ALERT_TRANSMIT_MSG_DISCARDED:
		SET_FLAG(prl_tx, PRL_FLAGS_TX_DISCARDED);
		break;
	case TCPC_ALERT_TRANSMIT_MSG_SUCCESS:
		SET_FLAG(prl_tx, PRL_FLAGS_TX_COMPLETE);
		break;
...
static uint32_t get_sop_star_header(const struct device *dev)
{
	...
	header.specification_revision = port->rev[prl_tx->emsg.type];
...
static void prl_tx_construct_message(const struct device *dev)
{
	 ...
	 */
	CLR_FLAG(prl_tx, PRL_FLAGS_TX_COMPLETE);

	/* Clear PRL_FLAGS_MSG_XMIT flag */
	CLR_FLAG(prl_tx, PRL_FLAGS_MSG_XMIT);
...
static void prl_hr_send_msg_to_phy(const struct device *dev)
{
	 ...
	 */
	port->prl_tx->flags = ATOMIC_INIT(0);
...
static void prl_init(const struct device *dev)
{
	 ...
	 */
	tcpc_set_alert_handler_cb(port->tcpc, alert_handler, (void *)dev);
...
static void prl_tx_wait_for_message_request_run(void *obj)
{
	...
	if (pe_dpm_initiated_ams(dev) == false) {
		CLR_FLAG(prl_tx, PRL_FLAGS_WAIT_SINK_OK);
	}
	 ...
	 * lines appropriately.
	 */
	if (port->rev[PD_PACKET_SOP] == PD_REV30 && pe_dpm_initiated_ams(dev)) {
		if (CHK_FLAG(prl_tx, PRL_FLAGS_WAIT_SINK_OK)) {
			/*
			 ...
			 * Policy Engine
			 */
			SET_FLAG(prl_tx, PRL_FLAGS_WAIT_SINK_OK);
	...
	if (CHK_FLAG(prl_tx, PRL_FLAGS_MSG_XMIT)) {
		/*
...
static void prl_tx_wait_for_phy_response_run(void *obj)
{
	...
	if (CHK_AND_CLR_FLAG(prl_tx, PRL_FLAGS_TX_DISCARDED)) {
	...
	if (CHK_FLAG(prl_tx, PRL_FLAGS_TX_COMPLETE)) {
		...
		return;
		   ...
		   CHK_FLAG(prl_tx, PRL_FLAGS_TX_ERROR)) {
		/*
...
static void prl_tx_snk_pending_run(void *obj)
{
	...
	/*
	 * The Protocol Layer Shall transition to the PRL_Tx_Layer_Reset_for_Transmit
	 * state when a Soft_Reset Message is pending.
	 */
	if ((prl_tx->msg_type == PD_CTRL_SOFT_RESET) && (prl_tx->emsg.len == 0)) {
		prl_tx_set_state(dev, PRL_TX_LAYER_RESET_FOR_TRANSMIT);
	}
	/*
	 * The Protocol Layer Shall transition to the PRL_Tx_Construct_Message state when Rp
	 * is set to SinkTxOk and a Soft_Reset Message is not pending.
	 */
 ...
 *	  NOTE: This state is not part of the PRL_HR State Diagram found in Figure 6-66. The PRL_HR
 *		state machine waits here until a Hard Reset is requested by either the Policy Engine
 *		or the PHY Layer.
 */
...
static void prl_hr_wait_for_request_run(void *obj)
{
	 ...
	 *
	 * The PRL_FLAGS_PORT_PARTNER_HARD_RESET flag is set when Hard Reset signaling
	 * is received by the PHY Layer.
	 */
	if (CHK_FLAG(prl_hr, PRL_FLAGS_PE_HARD_RESET) ||
	    CHK_FLAG(prl_hr, PRL_FLAGS_PORT_PARTNER_HARD_RESET)) {
		/* Start Hard Reset */
		prl_hr_set_state(dev, PRL_HR_RESET_LAYER);
	}
}
...
static void prl_hr_wait_for_phy_hard_reset_complete_entry(void *obj)
{
	...
	/*
	 * Start the HardResetCompleteTimer and wait for the PHY Layer to indicate that
	 * the Hard Reset completed.
	 */
...
static void prl_hr_wait_for_pe_hard_reset_complete_run(void *obj)
{
	...
	if (CHK_FLAG(prl_hr, PRL_FLAGS_HARD_RESET_COMPLETE)) {
		prl_hr_set_state(dev, PRL_HR_WAIT_FOR_REQUEST);
	}
}
...
/**
 * @brief This function implements both the Protocol Layer Message Reception State Machine.
 *	  See Figure 6-55 Protocol layer Message reception
 *
 *	  The states of the two state machines can be identified by the comments preceded
 *	  by a NOTE: <state name>
 */
static void prl_rx_wait_for_phy_message(const struct device *dev)
{
	struct usbc_port_data *port = dev->data;
	...
	if (msg_type != PD_CTRL_PING) {
		...
		LOG_PRINTK("RECV %04x/%d ", rx_emsg->header.raw_value, num_data_objs);
		for (p = 0; p < num_data_objs; p++) {
			LOG_PRINTK("[%d]%08x ", p, *((uint32_t *)rx_emsg->data + p));
		}
		LOG_PRINTK("\n");
	}
