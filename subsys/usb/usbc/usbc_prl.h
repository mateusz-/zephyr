...
struct protocol_layer_tx_t {
	...
	uint8_t msg_type;
	/** Power Delivery Messages meant for transmission are stored in emsg */
	struct pd_msg emsg;
...
void prl_send_ctrl_msg(const struct device *dev,
			   enum pd_packet_type type, enum pd_ctrl_msg_type msg);
...
void prl_send_data_msg(const struct device *dev,
			   enum pd_packet_type type, enum pd_data_msg_type msg);
