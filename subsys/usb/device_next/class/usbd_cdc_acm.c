...
#define CONFIG_USBD_CDC_ACM_STACK_SIZE	512
...
static int usbd_cdc_acm_ctd(struct usbd_class_node *const c_nd,
			    ...
			    struct usbd_req_compl *const req_compl)
{
	const struct device *dev = c_nd->data->priv;
	struct cdc_acm_uart_data *data = dev->data;
	size_t len;
	...
	case SET_LINE_CODING:
		...
		/* FIXME: update after status stage */
		cdc_acm_update_uart_cfg(data);
		return 0;

	case SET_CONTROL_LINE_STATE:
		/* FIXME: update line_state_dtr/rts after status stage */
...
static void cdc_acm_tx_fifo_handler(struct k_work *work)
{
	...
	len = MAX(ring_buf_size_get(data->tx_fifo.rb), buf->size);
	len = ring_buf_get(data->tx_fifo.rb, buf->data, len);
	net_buf_add(buf, len);
...
static void cdc_acm_irq_tx_enable(const struct device *dev)
{
	...
	if (ring_buf_is_empty(data->tx_fifo.rb) &&
	    !atomic_test_bit(&data->state, CDC_ACM_TX_FIFO_BUSY)) {
