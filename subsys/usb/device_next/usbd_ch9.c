...
static void ctrl_set_req_compl(struct usbd_contex *const uds_ctx,
			       void *const cb, void *const data)
{
	uds_ctx->ch9_data.req_compl.cb = cb;
	uds_ctx->ch9_data.req_compl.data = data;
}

static int sreq_set_address_cb(struct usbd_contex *const uds_ctx,
			       void *const data)
{
	...
	if (ret) {
		LOG_ERR("Failed to set device address %d", setup->wValue);
		return ret;
	}
	...
	LOG_DBG("Set new address %d", setup->wValue);
...
static int sreq_set_address(struct usbd_contex *const uds_ctx)
{
	...
	ctrl_set_req_compl(uds_ctx, sreq_set_address_cb, NULL);
...
{
	struct usbd_req_compl *req_compl = &uds_ctx->ch9_data.req_compl;
	struct usb_setup_packet *setup = usbd_get_setup_pkt(uds_ctx);
	...
	if (c_nd != NULL) {
		if (usb_reqtype_is_to_device(setup)) {
			ret = usbd_class_control_to_dev(c_nd, setup, dbuf, req_compl);
		} else {
...
static int ctrl_finish_request(struct usbd_contex *const uds_ctx)
{
	const struct usbd_req_compl *req_compl = &uds_ctx->ch9_data.req_compl;

	if (req_compl->cb != NULL) {
		return req_compl->cb(uds_ctx, req_compl->data);
	}

	LOG_WRN("Completion callback not set");
			  ...
			  struct net_buf *const buf, const int err)
{
	...
	if (bi->setup && bi->ep == USB_CONTROL_EP_OUT) {
		...
		/* Clear completion callback on new setup request */
		ctrl_set_req_compl(uds_ctx, NULL, NULL);
		...
		if (usb_reqtype_is_to_device(setup) && setup->wLength) {
			...
			/* Enqueue IN or STATUS (OUT) buffer */
			ret = usbd_ep_ctrl_enqueue(uds_ctx, next_buf);
		}
	...
	if (bi->status && bi->ep == USB_CONTROL_EP_IN) {
		if (ch9_get_ctrl_type(uds_ctx) == CTRL_AWAIT_STATUS_STAGE) {
			...
			ret = ctrl_finish_request(uds_ctx);
		} else {
