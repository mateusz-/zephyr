...
/**
 * @brief These macros SET, CLEAR, and CHECK, a Policy Engine Layer Flag
 */
#define SET_FLAG(obj, flag) atomic_set_bit(&obj->flags, flag)
#define CLR_FLAG(obj, flag) atomic_clear_bit(&obj->flags, flag)
#define CHK_FLAG(obj, flag) atomic_test_bit(&obj->flags, flag)
#define CHK_AND_CLR_FLAG(obj, flag) atomic_test_and_clear_bit(&obj->flags, flag)
...
enum pe_flags {
	...
	/** A hard reset has been requested by the DPM but has not been sent, not currently used */
	...
	/**
	 * Flag to note current Atomic Message Sequence (AMS) is interruptible. If this flag is
	 * not set the AMS is non-interruptible. This flag must be set in the interruptible's
	 * message state entry.
	 */
	...
	/** This flag is set when a Wait message is received in response to a Sink REQUEST */
	PE_FLAGS_WAIT_SINK_REQUEST      = 12,
	/** This flag is set when a Wait message is received in response to a Data Role Swap */
...
static void pe_set_state(const struct device *dev, enum usbc_pe_state state);
...
static void pe_send_soft_reset(const struct device *dev, enum pd_packet_type type);
static void policy_notify(const struct device *dev, enum policy_notify_t notify);
...
void pe_subsys_init(const struct device *dev)
{
	...
	struct policy_engine *pe = port->pe;
...
bool pe_is_running(const struct device *dev)
{
	...
	return port->pe_sm_state == SM_RUN;
}

/**
 * @brief Run the Policy Engine layer
 */
void pe_run(const struct device *dev, int32_t dpm_request)
{
	struct usbc_port_data *port = dev->data;
	struct policy_engine *pe = port->pe;

	switch (port->pe_sm_state) {
	case SM_PAUSED:
		if (port->pe_enabled == false) {
			break;
		}
	/* fall through */
	case SM_INIT:
		pe_init(dev);
		port->pe_sm_state = SM_RUN;
	/* fall through */
	case SM_RUN:
		if (port->pe_enabled == false) {
			port->pe_sm_state = SM_PAUSED;
			break;
		}
...
void pe_message_sent(const struct device *dev)
{
	...
	SET_FLAG(port->pe, PE_FLAGS_TX_COMPLETE);
}

/**
 * @brief Informs the Policy Engine of an error.
 */
void pe_report_error(const struct device *dev, enum pe_error e, enum pd_packet_type type)
{
	struct usbc_port_data *port = dev->data;
	...
	if ((e != ERR_XMIT && CHK_FLAG(port->pe, PE_FLAGS_INTERRUPTIBLE_AMS) == false)
	    || e == ERR_XMIT
	    || (CHK_FLAG(port->pe, PE_FLAGS_EXPLICIT_CONTRACT) == false &&
		type == PD_PACKET_SOP)) {
...
void pe_report_discard(const struct device *dev)
{
	 ...
	 */
	CLR_FLAG(port->pe, PE_FLAGS_DPM_INITIATED_AMS);
	SET_FLAG(port->pe, PE_FLAGS_MSG_DISCARDED);
}
...
void pe_message_received(const struct device *dev)
{
	...
	SET_FLAG(port->pe, PE_FLAGS_MSG_RECEIVED);
}
...
void pe_hard_reset_sent(const struct device *dev)
{
	...
	CLR_FLAG(port->pe, PE_FLAGS_HARD_RESET_PENDING);
}
...
bool pe_is_explicit_contract(const struct device *dev)
{
	...
	return CHK_FLAG(port->pe, PE_FLAGS_EXPLICIT_CONTRACT);
}
...
bool pe_dpm_initiated_ams(const struct device *dev)
{
	...
	return CHK_FLAG(port->pe, PE_FLAGS_DPM_INITIATED_AMS);
}
...
static void pe_set_state(const struct device *dev, enum usbc_pe_state state)
{
	...
	smf_set_state(SMF_CTX(port->pe), &pe_states[state]);
}
...
static enum usbc_pe_state pe_get_state(const struct device *dev)
{
	...
	return port->pe->ctx.current - &pe_states[0];
}
...
static enum usbc_pe_state pe_get_last_state(const struct device *dev)
{
	...
	return port->pe->ctx.previous - &pe_states[0];
}
 ...
 */
static inline void send_data_msg(const struct device *dev, enum pd_packet_type type,
				 enum pd_data_msg_type msg)
{
	struct usbc_port_data *port = dev->data;

	/* Clear any previous TX status before sending a new message */
	CLR_FLAG(port->pe, PE_FLAGS_TX_COMPLETE);
 ...
 */
static inline void send_ctrl_msg(const struct device *dev, enum pd_packet_type type,
 ...
 */
static void pe_send_request_msg(const struct device *dev, uint32_t rdo)
{
	struct usbc_port_data *port = dev->data;
	struct protocol_layer_tx_t *prl_tx = port->prl_tx;
	struct pd_msg *msg = &prl_tx->emsg;

	msg->len = sizeof(rdo);
	memcpy(msg->data, (uint8_t *)&rdo, msg->len);
...
{
	struct usbc_port_data *port = dev->data;
	struct protocol_layer_rx_t *prl_rx = port->prl_rx;
...
static bool common_dpm_requests(const struct device *dev)
{
	...
	if (pe->dpm_request > REQUEST_TC_END) {
		SET_FLAG(pe, PE_FLAGS_DPM_INITIATED_AMS);
...
static bool received_control_message(const struct device *dev,
				     union pd_header header, enum pd_ctrl_msg_type mt)
{
	struct usbc_port_data *port = dev->data;
	struct protocol_layer_rx_t *prl_rx = port->prl_rx;
...
static bool received_data_message(const struct device *dev,
				  union pd_header header, enum pd_data_msg_type mt)
{
	struct usbc_port_data *port = dev->data;
	struct protocol_layer_rx_t *prl_rx = port->prl_rx;
 ...
 */
static bool policy_check(const struct device *dev, enum policy_check_t pc)
{
	struct usbc_port_data *port = dev->data;

	if (port->policy_cb_check) {
		return port->policy_cb_check(dev, pc);
	} else {
		return false;
	}
}
...
static void policy_notify(const struct device *dev, enum policy_notify_t notify)
{
	...
	if (port->policy_cb_notify) {
		port->policy_cb_notify(dev, notify);
	}
}
 ...
 */
static bool policy_wait_notify(const struct device *dev, enum policy_wait_t notify)
{
	struct usbc_port_data *port = dev->data;

	if (port->policy_cb_wait_notify) {
		return port->policy_cb_wait_notify(dev, notify);
	}
 ...
 */
static void policy_set_src_cap(const struct device *dev, uint32_t *pdos, int num_pdos)
{
	struct usbc_port_data *port = dev->data;

	if (port->policy_cb_set_src_cap) {
		port->policy_cb_set_src_cap(dev, pdos, num_pdos);
	}
}
...
static uint32_t policy_get_request_data_object(const struct device *dev)
{
	...
	__ASSERT(port->policy_cb_get_request_data_object != NULL,
		 "Callback pointer should not be NULL");

	return port->policy_cb_get_request_data_object(dev);
}

/**
 * @brief Check if the sink is a default level
 */
static bool policy_is_snk_at_default_level(const struct device *dev)
{
	struct usbc_port_data *port = dev->data;

	if (port->policy_cb_is_snk_at_default_level) {
		return port->policy_cb_is_snk_at_default_level(dev);
	}
 ...
 */
static void policy_get_snk_cap(const struct device *dev, uint32_t **pdos, int *num_pdos)
{
	struct usbc_port_data *port = dev->data;

	/* This callback must be implemented */
	__ASSERT(port->policy_cb_get_snk_cap != NULL,
		 "Callback pointer should not be NULL");

	port->policy_cb_get_snk_cap(dev, pdos, num_pdos);
}
...
static void pe_snk_startup_entry(void *obj)
{
	...
	CLR_FLAG(pe, PE_FLAGS_EXPLICIT_CONTRACT);
...
static void pe_snk_wait_for_capabilities_run(void *obj)
{
	 ...
	 */
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_MSG_RECEIVED)) {
	...
	if (usbc_timer_expired(&pe->pd_t_typec_sink_wait_cap)) {
		SET_FLAG(pe, PE_FLAGS_SNK_WAIT_CAP_TIMEOUT);
		pe_set_state(dev, PE_SNK_HARD_RESET);
	}
}
...
static void pe_snk_select_capability_run(void *obj)
{
	...
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_MSG_DISCARDED)) {
		/*
	...
	} else if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_TX_COMPLETE)) {
	...
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_MSG_RECEIVED)) {
		...
		if (received_control_message(dev, header, PD_CTRL_ACCEPT)) {
			...
			SET_FLAG(pe, PE_FLAGS_EXPLICIT_CONTRACT);
			...
			/* We had a previous explicit contract, so transition to PE_SNK_Ready */
			if (CHK_FLAG(pe, PE_FLAGS_EXPLICIT_CONTRACT)) {
				if (received_control_message(dev, header, PD_CTRL_WAIT)) {
					/*
					 * Inform Device Policy Manager that Sink Request needs
					 * to Wait
					 */
					if (policy_wait_notify(dev, WAIT_SINK_REQUEST)) {
						SET_FLAG(pe, PE_FLAGS_WAIT_SINK_REQUEST);
...
static void pe_snk_transition_sink_run(void *obj)
{
	...
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_MSG_RECEIVED)) {
		...
		if (received_control_message(dev, header, PD_CTRL_PS_RDY)) {
			/* Inform the Device Policy Manager to Transition the Power Supply */
...
static void pe_snk_ready_entry(void *obj)
{
	...
	/* Clear AMS Flags */
	CLR_FLAG(pe, PE_FLAGS_INTERRUPTIBLE_AMS);
	CLR_FLAG(pe, PE_FLAGS_DPM_INITIATED_AMS);
}
...
static void pe_snk_ready_run(void *obj)
{
	...
	if (usbc_timer_expired(&pe->pd_t_wait_to_resend)) {
		if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_WAIT_SINK_REQUEST)) {
			pe_set_state(dev, PE_SNK_SELECT_CAPABILITY);
			return;
		} else if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_WAIT_DATA_ROLE_SWAP)) {
			pe_set_state(dev, PE_DRS_SEND_SWAP);
			return;
		}
	}
...
static void pe_snk_hard_reset_entry(void *obj)
{
	 ...
	 */
	if (CHK_FLAG(pe, PE_FLAGS_SNK_WAIT_CAP_TIMEOUT) &&
	...
	CLR_FLAG(pe, PE_FLAGS_SNK_WAIT_CAP_TIMEOUT);
	CLR_FLAG(pe, PE_FLAGS_PROTOCOL_ERROR);
...
static void pe_snk_hard_reset_run(void *obj)
{
	 ...
	 */
	if (CHK_FLAG(pe, PE_FLAGS_HARD_RESET_PENDING)) {
		return;
	}
...
static void pe_snk_transition_to_default_run(void *obj)
{
	...
	/* Wait until Device Policy Manager has transitioned the sink to default level */
	if (policy_is_snk_at_default_level(dev)) {
...
static void pe_snk_get_source_cap_run(void *obj)
{
	...
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_TX_COMPLETE)) {
		pe_set_state(dev, PE_SNK_READY);
	} else if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_MSG_DISCARDED)) {
		pe_send_soft_reset(dev, prl_rx->emsg.type);
	}
}
...
static void pe_send_soft_reset_entry(void *obj)
{
	...
	prl_reset(dev);
	SET_FLAG(pe, PE_FLAGS_SEND_SOFT_RESET);
}
...
static void pe_send_soft_reset_run(void *obj)
{
	...
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_SEND_SOFT_RESET)) {
	...
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_MSG_DISCARDED)) {
		...
		usbc_timer_start(&pe->pd_t_sender_response);
	} else if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_MSG_RECEIVED)) {
		header = prl_rx->emsg.header;

		if (received_control_message(dev, header, PD_CTRL_ACCEPT)) {
			pe_set_state(dev, PE_SNK_WAIT_FOR_CAPABILITIES);
		}
	} else if (CHK_FLAG(pe, PE_FLAGS_PROTOCOL_ERROR) ||
		   usbc_timer_expired(&pe->pd_t_sender_response)) {
		if (CHK_FLAG(pe, PE_FLAGS_PROTOCOL_ERROR)) {
			CLR_FLAG(pe, PE_FLAGS_PROTOCOL_ERROR);
		} else {
			policy_notify(dev, PORT_PARTNER_NOT_RESPONSIVE);
		}
...
static void  pe_soft_reset_run(void *obj)
{
	...
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_TX_COMPLETE)) {
		pe_set_state(dev, PE_SNK_WAIT_FOR_CAPABILITIES);
	} else if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_PROTOCOL_ERROR)) {
		pe_set_state(dev, PE_SNK_HARD_RESET);
	}
}
...
static void pe_send_not_supported_run(void *obj)
{
	...
	if (CHK_FLAG(pe, PE_FLAGS_TX_COMPLETE) || CHK_FLAG(pe, PE_FLAGS_MSG_DISCARDED)) {
		CLR_FLAG(pe, PE_FLAGS_TX_COMPLETE);
		CLR_FLAG(pe, PE_FLAGS_MSG_DISCARDED);
		pe_set_state(dev, PE_SNK_READY);
	}
}
...
static void pe_drs_evaluate_swap_entry(void *obj)
{
	...
	if (policy_check(dev, (pe->data_role == TC_ROLE_UFP) ?
		...
		/* PE_DRS_DFP_UFP_Accept_Swap and PE_DRS_UFP_DFP_Accept_Swap State embedded here */
		/* Send Accept message */
		send_ctrl_msg(dev, PD_PACKET_SOP, PD_CTRL_ACCEPT);
	} else {
		/* PE_DRS_DFP_UFP_Reject_Swap and PE_DRS_UFP_DFP_Reject_Swap State embedded here */
		/* Send Reject message */
...
static void pe_drs_evaluate_swap_run(void *obj)
{
	...
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_TX_COMPLETE)) {
		...
		if (prl_tx->msg_type == PD_CTRL_ACCEPT) {
			...
			tcpc_set_roles(port->tcpc, pe->power_role, pe->data_role);
...
static void pe_drs_send_swap_run(void *obj)
{
	...
	if (CHK_AND_CLR_FLAG(pe, PE_FLAGS_MSG_RECEIVED)) {
		...
		if (received_control_message(dev, header, PD_CTRL_REJECT)) {
			/* Inform Device Policy Manager that Data Role Swap Was Rejected */
			policy_notify(dev, MSG_REJECTED_RECEIVED);
		} else if (received_control_message(dev, header, PD_CTRL_WAIT)) {
			/* Inform Device Policy Manager that Data Role Swap needs to Wait */
			if (policy_wait_notify(dev, WAIT_DATA_ROLE_SWAP)) {
				SET_FLAG(pe, PE_FLAGS_WAIT_DATA_ROLE_SWAP);
				usbc_timer_start(&pe->pd_t_wait_to_resend);
			}
